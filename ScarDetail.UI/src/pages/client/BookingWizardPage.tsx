...
export const BookingWizardPage: React.FC = () => {
  ...
  const { data: slotsData, isLoading: loadingSlots } = useQuery({
    queryKey: ['available-slots', selectedPlan?.id, selectedDate],
    queryFn: () => appointmentService.getAvailableSlots(selectedPlan!.id, selectedDate),
  ...
  useEffect(() => {
    ...
    const fromLandingPage = services.find((service) =>
      service.id === serviceId || (serviceName && normalize(service.nome) === normalize(serviceName))
    );
    if (fromLandingPage) setSelectedPlan(fromLandingPage);
  }, [searchParams, selectedPlan, services]);

  const currentPrice = getPrice(selectedPlan, selectedVehicle?.categoria);
  ...
  const createAppointment = useMutation({
    mutationFn: () => appointmentService.createAppointment({
      planoId: selectedPlan!.id, veiculoId: selectedVehicle!.id,
      enderecoId: selectedAddress!.id, dataHoraInicio: selectedSlot!.dataHoraInicio,
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      {step === 1 && <section className={surface}>
        <SectionTitle step="Etapa 1 de 5" title="Qual serviço você quer?" description="O valor exato aparece após escolher o veículo." />
        {loadingServices ? <Loading text="Buscando serviços..." /> : <div className="grid gap-3 sm:grid-cols-2">{services.map((plan) => <button key={plan.id} type="button" onClick={() => setSelectedPlan(plan)} className={selection(selectedPlan?.id === plan.id)}><div className="flex items-start justify-between gap-3"><div><h3 className="font-black">{plan.nome}</h3><p className={`mt-1 text-sm leading-relaxed ${selectedPlan?.id === plan.id ? 'text-slate-300' : 'text-slate-500'}`}>{plan.descricao}</p></div><SelectionDot active={selectedPlan?.id === plan.id} /></div><div className={`mt-4 flex items-end justify-between border-t pt-3 ${selectedPlan?.id === plan.id ? 'border-white/15' : 'border-stone-100'}`}><span className="flex items-center gap-1.5 text-xs"><Clock3 className="h-4 w-4" />{formatDuration(plan.duracaoMinutos)}</span><strong>A partir de {plan.precoHatch.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</strong></div></button>)}</div>}
      ...
      {step === 2 && <section className={surface}>
        ...
        {selectedPlan && selectedVehicle && <PriceCallout plan={selectedPlan} price={currentPrice} />}
        <StepActions canContinue={!!selectedVehicle} onBack={() => setStep(1)} onNext={goNext} />
      </section>}

      {step === 3 && <section className={surface}>
        <SectionTitle step="Etapa 3 de 5" title="Quando fica melhor para você?" description="Mostramos somente horários livres para este serviço." />
      ...
      {step === 5 && selectedPlan && selectedVehicle && selectedSlot && selectedAddress && <section className={surface}>
        ...
        <div className="grid gap-3 sm:grid-cols-2"><SummaryItem icon={Sparkles} label="Serviço" value={selectedPlan.nome} detail={formatDuration(selectedPlan.duracaoMinutos)} /><SummaryItem icon={Car} label="Veículo" value={`${selectedVehicle.marca} ${selectedVehicle.modelo}`} detail={selectedVehicle.categoriaNomeExibicao} /><SummaryItem icon={CalendarDays} label="Data e horário" value={format(parseISO(selectedSlot.dataHoraInicio), "EEEE, dd 'de' MMMM", { locale: ptBR })} detail={`${selectedSlot.horarioInicioFormatado} às ${selectedSlot.horarioFimFormatado}`} /><SummaryItem icon={MapPin} label="Endereço" value={`${selectedAddress.logradouro}, ${selectedAddress.numero}`} detail={`${selectedAddress.bairro} · Curitiba/PR`} /></div>
        ...
        <div className="mt-5 flex flex-col gap-4 rounded-2xl bg-emerald-50 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Valor do atendimento</p><p className="text-3xl font-black text-slate-950">{currentPrice.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p><p className="mt-1 flex items-center gap-1.5 text-xs font-medium text-emerald-800"><CreditCard className="h-4 w-4" />PIX, cartão ou dinheiro após o serviço</p></div><button disabled={createAppointment.isPending} onClick={() => createAppointment.mutate()} className="rounded-2xl bg-emerald-600 px-6 py-4 font-black text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-500 disabled:opacity-50">{createAppointment.isPending ? 'Reservando...' : 'Confirmar agendamento'}</button></div>
...
const PriceCallout = ({ plan, price }: { plan: ServicePlan; price: number }) => <div className="mt-5 flex items-center justify-between rounded-2xl bg-amber-50 p-4"><div><p className="text-xs font-bold uppercase tracking-wider text-amber-800">{plan.nome}</p><p className="text-sm text-amber-900">Valor para este veículo</p></div><strong className="text-xl text-slate-950">{price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</strong></div>;
